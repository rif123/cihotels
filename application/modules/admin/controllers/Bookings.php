...
class Bookings extends Admin_Controller {
	...
	{		
		parent::__construct();
		$this->load->model(array('booking_model','room_type_model','guest_model', 'Building_model', 'Room_model'));
	...
	function add(){
		...
		$data['listGedung']	= $this->Building_model->get_all();
		$data['listCategory']	= $this->db->get('category')->result_array();
        ...
        {	
			...
			$this->form_validation->set_rules('room_no', 'Room No harus terisi', '');
			$this->form_validation->set_rules('idCategory', 'Kategory No harus terisi', '');
			...
			{	
				...
				$paid_services	=	$this->input->post('paid_services');	
					...
					$base_price	=	get_price_admin($_POST['check_in'],$_POST['check_out'],$_POST['room_type_id'],$_POST['adults'],$_POST['kids'], $_POST['idCategory']);
				...
				$booking_data['idBuilding']		=	$_POST['idBuilding'];
				$booking_data['idCategory']		=	$_POST['idCategory'];
				$booking_data['room_no']		=	$_POST['room_no'];
				...
				$booking_data['kegiatan']	=	$this->input->post('kegiatan');
				$booking_data['ordered_on']		=	date('Y-m-d H:i:s');
				$booking_data['base_price']		=	checkCategoryBooking($room_type->base_price, $this->input->post('idCategory'));
				...
				$booking_data['amount']			=	checkCategoryBooking(round($amount,2), $this->input->post('idCategory'));
				$booking_data['taxamount']		=	checkCategoryBooking(round($taxamount,2), $this->input->post('idCategory'));
				$booking_data['totalamount']	=	checkCategoryBooking(round($total,2), $this->input->post('idCategory'));
    ...
    function getKamar() {
        $idGedung = $_POST['idBuilding'];
        $listRoom = $this->Room_model->getByBuilding($idGedung);
        echo json_encode($listRoom);
    }
    function getRoomById() {
        $id = $_POST['id'];
        $listRoom = $this->Room_model->getByRoomNo($id);
        echo json_encode($listRoom);
    }
        ...
        {	
			...
			$coupon	=	strtolower($this->input->post('coupon'));
				...
				if(!empty($_POST['book'])){
					...
					$save['idBuilding']		=	$booking_data['idBuilding'];
					$save['idCategory']		=	$booking_data['idCategory'];
					$save['room_id']		=	$booking_data['room_no'];
					$save['kids']			=	$booking_data['kids'];
					$save['room_type_id']	=	$booking_data['room_type_id'];
					$save['kegiatan']	=	$booking_data['kegiatan'];
	...
	{	
		...
		$data['building'] = $this->booking_model->getBuildingWhere($data['rooms'][0]->idGedung);
		$data['getCategory'] = $this->booking_model->getCategoryData($data['booking']->idCategory);
		$data['getGuest'] = $this->booking_model->getGuestWhere($data['booking']->guest_id);
